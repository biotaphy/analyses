...
from ancestral_reconstruction.helpers.data_readers import (AlignmentIOError, 
                           read_csv_alignment_flo, read_json_alignment_flo,
                           read_phylip_alignment_flo, read_table_alignment_flo)
...
class Test_read_csv_alignment_flo(object):
    """
    @summary: Tests the read_csv_alignment_flo method
    """
    # .....................................
    def test_file_invalid(self, data_files):
        ...
        for fn in invalid_csv_alignments:
            with open(fn) as in_csv:
                with pytest.raises(AlignmentIOError):
                   read_csv_alignment_flo(in_csv)
    ...
    def test_file_valid(self, data_files):
        ...
        for fn in valid_csv_alignments:
            with open(fn) as in_csv:
                try:
                    sequence_list, headers = read_csv_alignment_flo(in_csv)
    ...
    def test_stringio_invalid(self, data_files):
        ...
        for fn in invalid_csv_alignments:
            with open(fn) as in_csv:
                ...
                csv_stringio.seek(0)
                with pytest.raises(AlignmentIOError):
                    read_csv_alignment_flo(csv_stringio)
    ...
    def test_stringio_valid(self, data_files):
        ...
        for fn in valid_csv_alignments:
            with open(fn) as in_csv:
                ...
                csv_stringio.seek(0)
                try:
                    sequence_list, headers = read_csv_alignment_flo(csv_stringio)
                    assert len(headers) > 0
                    assert len(sequence_list) > 0
                    for i in sequence_list:
                        assert isinstance(i, Sequence)
...
class Test_read_json_alignment_flo(object):
    """
    @summary: Tests the read_json_alignment_flo method
    """
    # .....................................
    def test_file_invalid(self, data_files):
        """
        ...
        for fn in invalid_json_alignments:
            with open(fn) as in_json:
                with pytest.raises(AlignmentIOError):
                    read_json_alignment_flo(in_json)
...
class Test_read_phylip_alignment_flo(object):
    """
    @summary: Tests the read_phylip_alignment_flo method
    """
    # .....................................
    def test_file_invalid(self, data_files):
        """
        ...
        for fn in invalid_phylip_alignments:
            with open(fn) as in_phylip:
                with pytest.raises(AlignmentIOError):
                    read_phylip_alignment_flo(in_phylip)
    ...
    def test_file_invalid(self, data_files):
        ...
        for fn in invalid_table_alignments:
            with open(fn) as in_table:
                with pytest.raises(AlignmentIOError):
                    read_table_alignment_flo(in_table)
